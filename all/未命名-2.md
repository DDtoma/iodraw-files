...
classDiagram
    ...
    class IMemoryManager {
        <<interface>>
        +store_memory(agent_id, memory_type, content) memory_id
        +retrieve_memory(agent_id, memory_type, memory_id) content
        +query_memories(agent_id, query) contents[]
    }
    
    class IMemoryStore {
        <<interface>>
        +save(agent_id, memory_type, content) memory_id
        +load(agent_id, memory_type, memory_id) content
        +batch_load(agent_id, memory_ids) contents[]
    }
    
    class IMemoryIndexer {
        <<interface>>
        +index_item(agent_id, memory_type, memory_id, content) void
        +search(agent_id, query) memory_ids[]
    }
    
    class IMemoryPolicy {
        <<interface>>
        +evaluate_async(agent_id, memory_type, memory_id) void
    }
    ...
    class AbstractMemoryManager {
        ...
        +retrieve_memory(agent_id, memory_type, memory_id)* content
        +query_memories(agent_id, query)* contents[]
        #_validate_access(agent_id) void
    }
    ...
    class AbstractMemoryStore {
        ...
        #_persist_async(agent_id, memory_type, memory_id, content) void
    }
    ...
    class AbstractMemoryIndexer {
        ...
        +index_item(agent_id, memory_type, memory_id, content)* void
        +search(agent_id, query)* memory_ids[]
        #_add_to_time_index(agent_id, timestamp, memory_id) void
        #_index_text(agent_id, memory_id, text) void
        #_extract_keywords(text) keywords[]
        #_search_by_time_range(agent_id, start_time, end_time) memory_ids[]
        #_search_by_keywords(agent_id, keywords) memory_ids[]
    }
    ...
    class AbstractMemoryPolicy {
        ...
        +evaluate_async(agent_id, memory_type, memory_id)* void
        #_manage_working_memory_capacity(agent_id) void
        #_consolidate_episodic_memories(agent_id) void
        #_apply_forgetting_strategy(agent_id) void
        #_forget_low_importance_memories(agent_id) void
