...
graph TB
    ...
    subgraph MetaCognition["元认知系统"]
        ...
        %% 目标维护
        GoalMaintainer["目标维护<br>1.任务约束分析<br>2.前置条件识别<br>3.任务边界确定"]
    end
    ...
    %% 进一步简化后的元认知系统到提示词系统的信息流
    GoalMaintainer -->|"任务约束条件"| ContextInjector
    ...
    %% 进一步简化后的元认知系统与定向阶段的交互
    GoalMaintainer -->|"任务约束与前置条件"| Orient
    
    %% 动机系统与决策系统的交互
    MotiveGenerator -->|"动机信号"| Decide
    GoalManager -->|"目标定义"| Decide
    ...
    class MetaCognition,GoalMaintainer MetaClass
